...
ansi_aware_handler <- function(x, options) {
  paste0("<pre class=\"r-output\"><code>", fansi::sgr_to_html(x = x, warn = FALSE, term.cap = "256"),"</code></pre>")}
knitr::knit_hooks$set(output = ansi_aware_handler, message = ansi_aware_handler, 
  warning = ansi_aware_handler,error = ansi_aware_handler)
```
...
<br><details>
<summary> Tests for `add_text_to_file()`. </summary>
...
<details>
<summary>Tests to make sure we remove temporary files on errors</summary>
...
<summary> Tests for `preprocess_chunk_labels()` `extract_rmd_code_chunks()` </summary>
...
We need to use `pkgload` to avoid a bug caused by how `devtools::install` unloads a package and we use `dplyr` for parsing the test output.
```{r}
...
```{r, results='asis'}
# need to comment this out to handle devtools::install's switch to using pkgload::unregister
# instead of pkgload::unload as of devtools@v2.4.4
# rm(list = ls())
litr::document()
install_old <- function() {
  remotes::install_github("jacobbien/litr-project@*release", subdir = "litr")
}
# we need to do some work to parse the test results
write_cat_results <- function(category, tests, ...) {
  cat("####", category, "\n")
  print(knitr::kable(tests))
}
write_file_results <- function(file, categories, ...) {
  cat("###", file, "\n")
  purrr::pwalk(list(categories$context, categories$Results), write_cat_results)
}
write_results <- function(res) {
  purrr::pwalk(list(res$file, res$Categories), write_file_results)
}
  
test_results <- xfun::Rscript_call(test_litr,
